    ...
    <div className="space-y-6">
      <FadeIn>
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            ...
            Select a pack to run its test scenarios.
          </p>
        </div>
      </FadeIn>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {available.map((pack, i) => (
          <FadeIn key={pack.id} delay={i * 0.05}>
            <Card className="relative overflow-hidden hover:border-emerald-500/30 transition-all group h-full flex flex-col">
              <CardHeader>
              ...
              <CardContent className="space-y-4 flex-1 flex flex-col">
                ...
                <div className="mt-auto pt-4 flex gap-2">
                  <Link href="/runner" className="flex-1">
                    <Button className="w-full bg-emerald-600 hover:bg-emerald-700 gap-1.5" size="sm">
        ...
        {coming.map((pack, i) => (
          <FadeIn key={pack.id} delay={(available.length + i) * 0.05}>
            <Card className="relative overflow-hidden opacity-60 h-full">
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="text-3xl mb-2">{pack.icon}</div>
